...
-(void)decodeFile:(NSString*)fileName fileExt:(NSString*)fileExt {
    ...
    //    NSLog(@"%@",h264FileSavePath);
    ////    NSURL *url = [NSURL fileURLWithPath:[[NSBundle mainBundle] pathForResource:@"10min" ofType:@"h264"]];
    //    NSLog(@"%@",[[NSBundle mainBundle] pathForResource:fileName ofType:fileExt]);
    [parser open:[[NSBundle mainBundle] pathForResource:@"10min" ofType:@"h264"]];//@"10min.h264"];//
    //    UIImageView * bigImage = [[UIImageView alloc]initWithImage:[UIImage imageWithContentsOfFile:[[NSBundle mainBundle] pathForResource:fileName ofType:fileExt]]];
    //    bigImage.frame = CGRectMake(0, 0, 320, 480);
    //    [self.view addSubview:bigImage];
    //[[NSBundle mainBundle] pathForResource:@"userdefault_img@2x" ofType:@"png"]
    VideoPacket *vp = nil;
    while(true) {
...
- (IBAction)playerAction:(id)sender {
    ...
    if (playCalled==true) {
        ...
        // 解码
        _glLayer = [[AAPLEAGLLayer alloc] initWithFrame:CGRectMake(0, 20, self.view.frame.size.width, self.view.frame.size.height-100  )] ;
        [self.view.layer addSublayer:_glLayer];
        
        dispatch_async(dispatch_get_global_queue(0, 0), ^{
            //            [self decodeFile:@"test" fileExt:@"h264"];
            [self decodeFile:@"datu" fileExt:@"jpg"];
        });
        return;
    }
    if(playCalled==false){
...
- (void) startCamera
{
    ...
    sbDisplayLayer.frame = CGRectMake(0, 20, self.view.frame.size.width, self.view.frame.size.height-100);
